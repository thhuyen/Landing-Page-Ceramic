...
const storage = (img, name, discount) => {
  ...
  if (discount.className.includes("discount")) {
    ...
    localStorage.setItem(
      "productDiscount",
      discount.nextElementSibling.innerText
    );
  } else {
...
const saveInfor1 = (product) => {
  storage(
    product.previousElementSibling.childNodes[1].src,
    product.innerText,
    product.nextElementSibling.nextElementSibling
  );
};
const saveInfor = (product, id) => {
  localStorage.setItem("selectId", id);
  storage(
    product.childNodes[1].src,
    product.nextElementSibling.innerText,
    product.nextElementSibling.nextElementSibling.nextElementSibling
  );
};
